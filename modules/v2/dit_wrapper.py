...
from modules.v2.trus import make_trus_hook
...
class DiT(torch.nn.Module):
    def __init__(
        ...
        self.trus_manager = None
        self.trus_hook_handles = []
    ...
    def set_trus_manager(self, trus_manager):
        self.clear_trus_hooks()
        self.trus_manager = trus_manager
        if trus_manager is None:
            return

        for layer_idx, block in enumerate(self.transformer.layers):
            h = block.feed_forward.register_forward_hook(
                make_trus_hook(trus_manager, layer_idx)
            )
            self.trus_hook_handles.append(h)

    def clear_trus_hooks(self):
        for h in self.trus_hook_handles:
            h.remove()
        self.trus_hook_handles = []